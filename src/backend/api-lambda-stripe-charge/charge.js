const AWS = require('aws-sdk'),
  ...
  STRIPE_SECRET_KEY_NAME1 = `/${process.env.SSM_PARAMETER_PATH1}`,
  STRIPE_SECRET_KEY_NAME2 = `/${process.env.SSM_PARAMETER_PATH2}`,
  IS_CORS = true;
...
exports.handler = (event) => {
  ...
  cas = Math.floor(Math.random() * 2)
  if (cas == 0) {
	  return ssm.getParameter({ Name: STRIPE_SECRET_KEY_NAME1, WithDecryption: true }).promise()
		.then(response => {
	  ...
	  return ssm.getParameter({ Name: STRIPE_SECRET_KEY_NAME2, WithDecryption: true }).promise()
		.then(response => {
		  const stripeSecretKeyValue = response.Parameter.Value;
		  return createCharge(stripeSecretKeyValue, chargeRequest.stripeToken, chargeRequest.email, chargeRequest.amount, chargeRequest.currency, chargeRequest.description);
		})
